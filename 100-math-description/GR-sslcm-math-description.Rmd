...
### Egg Production, Parr Recruitment, and Parr Mean Size
...
and the realized mean parr length as being multivariate log-normally distributed around $\dot{L}^{P^{b}}_{y,j}$ with covariance matrix $\Sigma_{L^{P^{b}}}$ (constructed of $\sigma_{L_{j}^{P^{b}}}$ and $\rho_{L^{P^{b}}}$ following eq. \@ref(eq:generic-Sigma4by4)):
...
### Survival to Smolt Stage and Mean Smolt Size

Overwinter survival from the end of the parr year (i.e., age-1) to the start of the smolt year (i.e., age-2), denoted by $\phi^{P^{a} \rightarrow M^{b}}_{y,i,j}$, was modeled as a logit-linear function of mean parr size.
...
Where $L^{*P^{b}}_{y,j}$ has been scaled and centered by subtracting the historical average observed mean parr length for population $j$ from $L^{P^{b}}_{y,j}$ and dividing by the historical observed standard deviation.
...
Because only spring migrants are measured for mean smolt length in the spring (as they pass the screw trap), we must assume equal lengths for fall migrant smolt and thus omit the $i$ subscript from these quantities related to mean smolt length.
Preliminary analyses on the observed mean length data revealed that mean smolt length was positively related with parr length, but that the relationship was non-linear.
We thus modeled the expected multiplicative change in mean size between the parr and smolt stage as a log-linear function of (standardized) parr mean size:
...
Mean smolt length was then the product:
...
For populations with hatchery supplementation (CAT, LOS, and UGR), hatchery-origin smolt releases (assumed known without error) were stored in the variable $M^{b}_{y,i=\mathrm{spring},o=\mathrm{HOR},j}$; we assumed no hatchery-origin parr were released the prior year, thus $M^{b}_{y,i=\mathrm{fall},o=\mathrm{HOR},j} = 0$.
The abundance of hatchery-origin fish for all subsequent life stages was tracked separately from natural-origin fish.
...
We modeled deterministic out-of-basin migration for natural-origin smolt as a logit-linear function of smolt size:
...
Where $L^{*M^{b}}_{y,j}$ has been scaled and centered by subtracting the historical average observed mean smolt length for population $j$ from $L^{M^{b}}_{y,j}$ and dividing by the historical observed standard deviation (just as for $L^{*P^{b}}_{y,j}$ for standardized parr mean length).
Realized out-of-basin migration survival was a multivariate logit-normal random variable around the deterministic logistic model prediction with covariance matrix $\Sigma_{\phi^{M^{b} \rightarrow M^{a}}}$ (each matrix constructed of $\sigma_{\phi^{M^{b} \rightarrow M^{a}}_{j}}$ and $\rho_{\phi^{M^{b} \rightarrow M^{a}}}$ following eq. \@ref(eq:generic-Sigma4by4)):

\begin{equation}
\logit\left(\boldsymbol{\phi}^{M^{b} \rightarrow M^{a}}_{y,o,1:n_j}\right) \sim \MVNdist\left[\logit\left(\boldsymbol{\dot{\phi}}^{M^{b} \rightarrow M^{a}}_{o,1:n_j}\right), \Sigma_{\phi^{M^{b} \rightarrow M^{a}}_o}\right]
(\#eq:phi-Mb-Ma-random)
\end{equation}
