...
\begin{equation}
M^{b}_{y,i,o=\mathrm{NOR},j} = P^{a}_{y,i,j} \cdot \phi^{P^{a} \to M^{b}}_{y,i,j}
(\#eq:get-Mb-NOR)
\end{equation}
...
\begin{equation}
\logit\left(\dot{\phi}^{M^{b} \to M^{a}}_{y,o=\mathrm{NOR},j}\right) = \tau_{0,j} + \tau_{1,j} \cdot L^{*M^{b}}_{y,j}
...
For hatchery-origin smolt, we assumed this expected out-of-basin survival term was time-constant (i.e., all $\dot{\phi}^{M^{b} \to M^{a}}_{y,o=\mathrm{HOR},j}$ were equal within a population).
...
\begin{equation}
\logit\left(\hat{\phi}^{P^{b} \to M^{a}}_{y,j}\right) \sim \Ndist\left[\logit\left(\frac{\sum_{i}^{n_i} M^{a}_{y,i,o=\mathrm{NOR},j}}{P^{b}_{y,j}}\right),\hat{\sigma}_{\phi^{P^{b} \to M^{a}}_{y,j}}\right]
(\#eq:summer-surv-data)
\end{equation}
...
\begin{equation}
\logit\left(\hat{\phi}^{P^{a} \to M^{a}}_{y,i=\mathrm{fall},j}\right) \sim \Ndist\left[\logit\left(\frac{M^{a}_{y,i=\mathrm{fall},o=\mathrm{NOR},j}}{P^{a}_{y,i=\mathrm{fall},j}}\right),\hat{\sigma}_{\phi^{P^{a} \to M^{a}}_{y,i=\mathrm{fall},j}}\right]
(\#eq:fall-surv-data)
\end{equation}
...
\begin{equation}
\logit\left(\hat{\phi}^{P^{a} \to M^{a}}_{y,i=\mathrm{spring},j}\right) \sim \Ndist\left[\logit\left(\frac{M^{a}_{y,i=\mathrm{spring},o=\mathrm{NOR},j}}{P^{a}_{y,i=\mathrm{spring},j} \cdot \phi^{P^{b} \to P^{a}}_{j}}\right),\hat{\sigma}_{\phi^{P^{a} \to M^{a}}_{y,i=\mathrm{spring},j}}\right]
(\#eq:winter-surv-data)
\end{equation}

for the winter tagging group (where $\phi^{P^{b} \to P^{a}}_j$ is a time-constant model-estimated survival from summer tagging to winter tagging for parr that ultimately become spring migrants and $j \in [\mathrm{CAT}, \mathrm{LOS}, \mathrm{UGR}]$), 

\begin{equation}
\logit\left(\hat{\phi}^{M^{b} \to M^{a}}_{y,i=\mathrm{spring},o=\mathrm{NOR},j}\right) \sim \Ndist\left[\logit\left(\phi^{M^{b} \to M^{a}}_{y,i=\mathrm{spring},o=\mathrm{NOR},j}\right),\hat{\sigma}_{\phi^{M^{b} \to M^{a}}_{y,i=\mathrm{spring},o=\mathrm{NOR},j}}\right]
(\#eq:spring-surv-data)
\end{equation}
...
\begin{equation}
\logit\left(\hat{\phi}^{M^{b} \to M^{a}}_{y,i=\mathrm{spring},o=\mathrm{HOR},j}\right) \sim \Ndist\left[\logit\left(\phi^{M^{b} \to M^{a}}_{y,i=\mathrm{spring},o=\mathrm{HOR},j}\right),\hat{\sigma}_{\phi^{M^{b} \to M^{a}}_{y,i=\mathrm{spring},o=\mathrm{HOR},j}}\right]
...
We assumed process variation was negligible in the second and third year ocean survival terms, thus we forced the year-specific values of $\phi^{O^{1} \to O^{2}}_{y,o=\mathrm{NOR},j}$ and $\phi^{O^{2} \to O^{3}}_{y,o=\mathrm{NOR},j}$ to take on the same value for all years (i.e., $\dot{\phi}^{O^{1} \to O^{2}}_{o=\mathrm{NOR},j}$ and $\dot{\phi}^{O^{2} \to O^{3}}_{o=\mathrm{NOR},j}$, respectively).
Because not all maturity and ocean survival terms were uniquely identifiable based on adult return abundance and age composition, we relied on reasonably strong priors to inform $\dot{\phi}^{O^{1} \to O^{2}}_{o=\mathrm{NOR},j}$ and $\dot{\phi}^{O^{2} \to O^{3}}_{o=\mathrm{NOR},j}$ (Table \@ref(tab:param-table)).
...
\begin{equation}
\begin{aligned}
\logit\left(\phi^{O^{0} \to O^{1}}_{y,o=\mathrm{HOR},j}\right) &= \logit\left(\phi^{O^{0} \to O^{1}}_{y,o=\mathrm{NOR},j}\right) + \delta_j \\
\logit\left(\phi^{O^{1} \to O^{2}}_{y,o=\mathrm{HOR},j}\right) &= \logit\left(\phi^{O^{1} \to O^{2}}_{y,o=\mathrm{NOR},j}\right) + \delta_j \\
\logit\left(\phi^{O^{2} \to O^{3}}_{y,o=\mathrm{HOR},j}\right) &= \logit\left(\phi^{O^{2} \to O^{3}}_{y,o=\mathrm{NOR},j}\right) + \delta_j \\
\end{aligned}
(\#eq:get-phi-O-HOR)
\end{equation}
