...
fit_basic_BH = function(jags_data, plot = FALSE) {
  out = with(jags_data, {
    ...
    Ma_obs = Pa_obs[,"fall-mig"] * expit(Lphi_obs_Pa_Ma[,"fall-mig"]) + 
      Mb_obs[,"spring-mig"] * expit(Lphi_obs_Mb_Ma[,"spring-mig"])
...
gen_initials = function(c, jags_data) {
  ...
  BH_fit = fit_basic_BH(jags_data, plot = FALSE)$BH_fit
  ...
  list(
    log_alpha = rnorm(1, BH_ests$coef["log_alpha","Estimate"], BH_ests$coef["log_alpha","Std. Error"]),
