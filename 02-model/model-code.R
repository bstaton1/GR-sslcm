jags_model_code = function() {
  ...
  Tau_Lphi_E_Pb[1:nj,1:nj] ~ dscaled.wishart(rep(0.3, nj), isw_df)
  
  # parr mean length
  Tau_lL_Pb[1:nj,1:nj] ~ dscaled.wishart(rep(0.1, nj), isw_df)

  # proportion of parr that are fall migrants
  Tau_Lpi[1:nj,1:nj] ~ dscaled.wishart(rep(0.3, nj), isw_df)

  # overwinter survival
  Tau_Lphi_Pa_Mb[1:nj,1:nj] ~ dscaled.wishart(rep(0.3, nj), isw_df)

  # overwinter change in mean size
  Tau_lDelta_L_Pb_Mb[1:nj,1:nj] ~ dscaled.wishart(rep(0.1, nj), isw_df)

  # migration survival to LGR
  Tau_Lphi_Mb_Ma[1:nj,1:nj] ~ dscaled.wishart(rep(0.3, nj), isw_df)

  # migration survival from LGR to ocean
  Tau_Lphi_Ma_O0[1:no,1:no] ~ dscaled.wishart(rep(0.3, no), isw_df)
  
  # Yr1 ocean survival
  Tau_Lphi_O0_O1[1:nj,1:nj] ~ dscaled.wishart(rep(0.3, nj), isw_df)
  
  # Pr(mature after 1 year at sea)
  Tau_Lpsi_O1[1:nj,1:nj] ~ dscaled.wishart(rep(0.15, nj), isw_df)
  
  # Pr(mature after 2 years at sea)
  Tau_Lpsi_O2[1:nj,1:nj] ~ dscaled.wishart(rep(0.3, nj), isw_df)
  
  # migration survival BON to LGR
  Tau_Lphi_Rb_Ra[1:no,1:no] ~ dscaled.wishart(rep(0.3, no), isw_df)
  
  # pre-spawn survival
  Tau_Lphi_Sb_Sa[1:nj,1:nj] ~ dscaled.wishart(rep(0.3, nj), isw_df)
  ...
  for (y in 2:ny) {
    ...
    lL_Pb[y,1:nj] ~ dmnorm(omega0[1:nj] + omega1[1:nj] * log((E[y,1:nj]/E_scale)/wul[1:nj]), Tau_lL_Pb[1:nj,1:nj])
    
    # LH apportionment
    Lpi1[y,1:nj] ~ dmnorm(logit(mu_pi[i_fall,1:nj]), Tau_Lpi[1:nj,1:nj])

    # overwinter survival by LH type: function of size
    Lphi_Pa_Mb[y,i_fall,1:nj] ~ dmnorm(gamma0[i_fall,1:nj] + gamma1[i_fall,1:nj] * L_Pb_star[y,1:nj], Tau_Lphi_Pa_Mb[1:nj,1:nj])
    Lphi_Pa_Mb[y,i_spring,1:nj] ~ dmnorm(gamma0[i_spring,1:nj] + gamma1[i_spring,1:nj] * L_Pb_star[y,1:nj], Tau_Lphi_Pa_Mb[1:nj,1:nj])

    # summer to spring growth factor
    lDelta_L_Pb_Mb[y,1:nj] ~ dmnorm(theta0[1:nj] + theta1[1:nj] * L_Pb_star[y,1:nj], Tau_lDelta_L_Pb_Mb[1:nj,1:nj])
    
    # size-based migration survival from trib to LGR: NOR
    Lphi_Mb_Ma[y,i_spring,o_nor,1:nj] ~ dmnorm(tau0[1:nj] + tau1[1:nj] * L_Mb_star[y,1:nj], Tau_Lphi_Mb_Ma[1:nj,1:nj])
    ...
    Lphi_O0_O1[y,o_nor,1:nj] ~ dmnorm(logit(mu_phi_O0_O1[o_nor,1:nj]) + Lphi_O0_O1_resid[y-1,o_nor,1:nj] * kappa_phi_O0_O1[1:nj], Tau_Lphi_O0_O1[1:nj,1:nj])
