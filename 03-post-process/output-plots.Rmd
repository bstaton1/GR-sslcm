...
plot_tseries(post_summ(post, "^phi_Ma_M[.+,1]$"), expit(jags_data$Lphi_obs_Ma_M[observable,1]), main = TeX("$LGR\\,Smolt \\rightarrow BON\\,(NOR)"))
plot_tseries(post_summ(post, "^phi_Ma_M[.+,2]$"), expit(jags_data$Lphi_obs_Ma_M[observable,2]), main = TeX("$LGR\\,Smolt \\rightarrow BON\\,(HOR)"))
mtext(side = 1, outer = T, line = 0.25, "Brood Year")
...
get_SAR_post = function(post, jags_data) {
  ...
  for (i in 1:ni) {
    ...
    # give names
    dimnames(Ra) = dimnames(jags_data$n_broodstock)
...
plot_tseries(post_summ(post, "phi_M_O1[.+,1]$"), main = TeX("$O_0 \\rightarrow O_1\\,(NOR)$"), yrs = as.numeric(rownames(jags_data$Pa_obs)[observable]), ylim = c(0,1))
plot_tseries(post_summ(post, "phi_M_O1[.+,2]$"), main = TeX("$O_0 \\rightarrow O_1\\,(HOR)$"), yrs = as.numeric(rownames(jags_data$Pa_obs)[observable]), ylim = c(0,1))
...
plot_tseries(post_summ(post, "^omega[.+,1,1]$"), main = "Natural Origin", yrs = as.numeric(rownames(jags_data$Pa_obs)[observable]), ylim = c(0,1))
...
alpha_summ = post_summ(post, "alpha", digits = 0)
beta_summ = post_summ(post, "beta", digits = -3)
sigma_summ = post_summ(post, "sigma_Pb", digits = 2)

# create/format the table
tab = cbind(alpha_summ, beta_summ, sigma_summ)
colnames(tab) = c("$\\alpha$", "$\\beta$", "$\\sigma_{Pb}$")
...
This figure shows the frequency that `Sb[y,k,s,o]` (spawners passed above weir, following broodstock removals) would go negative if not for the constraint `Sb[y,k,s,o] <- max(Ra[y,k,s,o] - n_broodstock[y,k,s,o], 0)`.

```{r, fig.width = 8, fig.height = 5}
Sb_post = post_subset(post, "^Sb[", matrix = TRUE)
p0 = array_format(colMeans(Sb_post == 0))
dimnames(p0) = dimnames(jags_data$n_broodstock)
years = as.numeric(dimnames(p1)[[1]])

par(mfcol = c(3,4), oma = c(1.5,1.5,0,0), mar = c(1.5,1.5,1.5,0.5), tcl = -0.15, mgp = c(2,0.35,0))
plot(p0[observable,1,1,1] ~ years[observable], type = "l", main = TeX("$Female\\,Age\\,3\\,(NOR)$"), ylim = c(0,1))
plot(p0[observable,2,1,1] ~ years[observable], type = "l", main = TeX("$Female\\,Age\\,4\\,(NOR)$"), ylim = c(0,1))
plot(p0[observable,3,1,1] ~ years[observable], type = "l", main = TeX("$Female\\,Age\\,5\\,(NOR)$"), ylim = c(0,1))
plot(p0[observable,1,2,1] ~ years[observable], type = "l", main = TeX("$Male\\,Age\\,3\\,(NOR)$"), ylim = c(0,1))
plot(p0[observable,2,2,1] ~ years[observable], type = "l", main = TeX("$Male\\,Age\\,4\\,(NOR)$"), ylim = c(0,1))
plot(p0[observable,3,2,1] ~ years[observable], type = "l", main = TeX("$Male\\,Age\\,5\\,(NOR)$"), ylim = c(0,1))
plot(p0[observable,1,1,2] ~ years[observable], type = "l", main = TeX("$Female\\,Age\\,3\\,(HOR)$"), ylim = c(0,1))
plot(p0[observable,2,1,2] ~ years[observable], type = "l", main = TeX("$Female\\,Age\\,4\\,(HOR)$"), ylim = c(0,1))
plot(p0[observable,3,1,2] ~ years[observable], type = "l", main = TeX("$Female\\,Age\\,5\\,(HOR)$"), ylim = c(0,1))
plot(p0[observable,1,2,2] ~ years[observable], type = "l", main = TeX("$Male\\,Age\\,3\\,(HOR)$"), ylim = c(0,1))
plot(p0[observable,2,2,2] ~ years[observable], type = "l", main = TeX("$Male\\,Age\\,4\\,(HOR)$"), ylim = c(0,1))
plot(p0[observable,3,2,2] ~ years[observable], type = "l", main = TeX("$Male\\,Age\\,5\\,(HOR)$"), ylim = c(0,1))
```
