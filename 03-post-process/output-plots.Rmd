...
L2 = c("Intercept", "Slope", "Process SD", "Among-pop Correlation", "Realized")
match = c("int_lgrowth", "slp_lgrowth", "sig_lgrowth", "rho_lgrowth", "^growth[")
tmp_diags = lapply(1:length(match), function(i) summarize_posterior(match[i], L1 = L1, L2 = L2[i]))
tmp_summ = lapply(tmp_diags, function(i) summarize_diags(i))
diags_out = rbind(diags_out, do.call(rbind, tmp_diags))
summs_out = rbind(summs_out, do.call(rbind, tmp_summ))
...
plot_f = function(j) {
  ...
  growth_mean = post_summ(post, sub_index("^growth[year,pop]", year = ts_yrs, pop = j))["mean",]
  growth_lwr = post_summ(post, sub_index("^growth[year,pop]", year = ts_yrs, pop = j))["2.5%",]
  growth_upr = post_summ(post, sub_index("^growth[year,pop]", year = ts_yrs, pop = j))["97.5%",]
  ...
  # extract posteriors of coefficients
  int_lgrowth = post_subset(post, sub_index("int_lgrowth[pop]", pop = j), matrix = TRUE)
  slp_lgrowth = post_subset(post, sub_index("slp_lgrowth[pop]", pop = j), matrix = TRUE)

  # function to create predicted curves for one posterior sample
  pred_fn = function(i) {
    pred_growth = exp(int_lgrowth[i] + slp_lgrowth[i] * L_Pb_star_seq)
    names(pred_growth) = paste0("pred_growth[", 1:30, "1]")
    pred_growth
  }
  
  # calculate predicted growth values and add to posterior samples
  pred_growth = t(sapply(1:post_dim(post, "saved"), pred_fn))
  post_pred_growth = post_convert(cbind(postpack:::id_mat(post), pred_growth))
  
  # extract summarize predicted growth
  pred_growth_mean = post_summ(post_pred_growth, "")["mean",]
  pred_growth_lwr = post_summ(post_pred_growth, "")["2.5%",]
  pred_growth_upr = post_summ(post_pred_growth, "")["97.5%",]
  
  # make a blank plot with the correct dimensions
  par(mgp = c(2,0.35, 0), tcl = -0.15, mar = c(1,1,2,1))
  plot(1,1, type = "n", 
       ylim = range(pred_growth_lwr, pred_growth_upr, growth_lwr, growth_upr, na.rm = TRUE),
       xlim = range(L_Pb_seq), 
       main = pops[j]
  )
    
  # expected relationship
  polygon(c(L_Pb_seq, rev(L_Pb_seq)), c(pred_growth_lwr, rev(pred_growth_upr)), border = NA, col = alpha("salmon", 0.5))
  lines(pred_growth_lwr ~ L_Pb_seq, col = "red", lty = 2)
  lines(pred_growth_upr ~ L_Pb_seq, col = "red", lty = 2)
  lines(pred_growth_mean ~ L_Pb_seq, col = "red", lwd = 2)
  
  # realized pairs
  segments(L_Pb_lwr, growth_mean, L_Pb_upr, growth_mean, col = alpha("red", 0.5))
  segments(L_Pb_mean, growth_lwr, L_Pb_mean, growth_upr, col = alpha("red", 0.5))
  points(growth_mean ~ L_Pb_mean, pch = 21, col = "red", bg = alpha("salmon", 0.5), cex = 1.2)
...
rho_names = c(
  ...
  "rho_lgrowth[2,1]" = "Summer to Spring 'Growth Factor'",
...
proc_error_ests$param = stringr::str_replace(proc_error_ests$param, "lgrowth_resid\\[year,pop\\]", "Summer to Spring 'Growth Factor'")
