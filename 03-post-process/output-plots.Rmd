...
```{r make-diag-post, message = FALSE}
...
if (length(simulated) > 0) {
  diag_post = post_remove(post, paste0("[", simulated), ask = FALSE)
} else {
  diag_post = post
}
```
...
L2 = c("BH Productivity", "BH Capacity", "Capacity vs. Habitat Slope", "Capacity vs. slope SD", "Process SD", "Among-pop Correlation", "Realized Survival", "Parr States")
match = c("alpha", "^beta[", "mu_beta_per_wul", "sig_lbeta", "sig_Lphi_f_Pb", "rho_Lphi_f_Pb", "^phi_f_Pb[", "^Pb[")
...
match = c("phi_Pb_Pa[2,.]")
phi_Pb_Pa_diags = unlist_dfs(lapply(1:length(match), function(i) diag_fn(match[i], L1 = L1)))
...
resids = lapply(1:4, get_resids, post = diag_post)
...
plot_f = function(j) {
  plot_tseries(post_summ(post, sub_index("^phi_Pa_Ma[year,LH_type,pop]$", year = observable, LH_type = i_spring, pop = j)),
...
# containers
x_weir_age_obs = x_carc_age_obs = array(NA, dim = c(jags_data$ny_obs, jags_data$nk, jags_data$nj))
x_weir_origin_obs = x_carc_origin_obs = array(NA, dim = c(jags_data$ny_obs, jags_data$no, jags_data$nj))
...
weir_comp_fit_plot = function(j) {
  ...
  plot_tseries(post_summ(post, sub_index("^q_Ra[year,1,pop]$", pop = j, year = ts_yrs)), get_obs_ests_multinomial(jags_data$weir_x_obs[observable,,j], 1), main = TeX("$Age\\,3\\,(NOR)$"), yrs = all_yrs[ts_yrs])
  plot_tseries(post_summ(post, sub_index("^q_Ra[year,2,pop]$", pop = j, year = ts_yrs)), get_obs_ests_multinomial(jags_data$weir_x_obs[observable,,j], 2), main = TeX("$Age\\,4\\,(NOR)$"), yrs = all_yrs[ts_yrs])
  plot_tseries(post_summ(post, sub_index("^q_Ra[year,3,pop]$", pop = j, year = ts_yrs)), get_obs_ests_multinomial(jags_data$weir_x_obs[observable,,j], 3), main = TeX("$Age\\,5\\,(NOR)$"), yrs = all_yrs[ts_yrs])
  plot_tseries(post_summ(post, sub_index("^q_Ra[year,4,pop]$", pop = j, year = ts_yrs)), get_obs_ests_multinomial(jags_data$weir_x_obs[observable,,j], 4), main = TeX("$Age\\,3\\,(HOR)$"), yrs = all_yrs[ts_yrs])
  plot_tseries(post_summ(post, sub_index("^q_Ra[year,5,pop]$", pop = j, year = ts_yrs)), get_obs_ests_multinomial(jags_data$weir_x_obs[observable,,j], 5), main = TeX("$Age\\,4\\,(HOR)$"), yrs = all_yrs[ts_yrs])
  plot_tseries(post_summ(post, sub_index("^q_Ra[year,6,pop]$", pop = j, year = ts_yrs)), get_obs_ests_multinomial(jags_data$weir_x_obs[observable,,j], 6), main = TeX("$Age\\,5\\,(HOR)$"), yrs = all_yrs[ts_yrs])
...
weir_comp_fit_plot_age = function(j) {
  ...
  plot_tseries(post_summ(post, sub_index("^q_Ra_age[year,1,pop]$", pop = j, year = ts_yrs)), get_obs_ests_multinomial(x_weir_new_obs$x_weir_age_obs[observable,,j], 1), main = TeX("$Age\\,3$"), ylim = c(0,1), yrs = all_yrs[ts_yrs])
  plot_tseries(post_summ(post, sub_index("^q_Ra_age[year,2,pop]$", pop = j, year = ts_yrs)), get_obs_ests_multinomial(x_weir_new_obs$x_weir_age_obs[observable,,j], 2), main = TeX("$Age\\,4$"), ylim = c(0,1), yrs = all_yrs[ts_yrs])
  plot_tseries(post_summ(post, sub_index("^q_Ra_age[year,3,pop]$", pop = j, year = ts_yrs)), get_obs_ests_multinomial(x_weir_new_obs$x_weir_age_obs[observable,,j], 3), main = TeX("$Age\\,5$"), ylim = c(0,1), yrs = all_yrs[ts_yrs])
  mtext(side = 1, outer = T, line = 0.25, "Return Year")
  mtext(side = 2, outer = T, line = 0.25, "Proportion of Return")
}
...
weir_comp_fit_plot_origin = function(j) {
  ...
  plot_tseries(post_summ(post, sub_index("^q_Ra_origin[year,1,pop]$", pop = j, year = ts_yrs)), get_obs_ests_multinomial(x_weir_new_obs$x_weir_origin_obs[observable,,j], 1), main = TeX("$NOR$"), ylim = c(0,1), yrs = all_yrs[ts_yrs])
  plot_tseries(post_summ(post, sub_index("^q_Ra_origin[year,2,pop]$", pop = j, year = ts_yrs)), get_obs_ests_multinomial(x_weir_new_obs$x_weir_origin_obs[observable,,j], 2), main = TeX("$HOR$"), ylim = c(0,1), yrs = all_yrs[ts_yrs])
  mtext(side = 1, outer = T, line = 0.25, "Return Year")
  mtext(side = 2, outer = T, line = 0.25, "Proportion of Return")
}
```
...
carc_comp_fit_plot = function(j) {
  ...
  par(mfrow = c(2,3), oma = c(1.5,1.5,0,0))#, mar = c(1.5,1.5,1.5,0.5))
  plot_tseries(post_summ(post, sub_index("^q_Sa_adj[year,1,pop]$", pop = j, year = ts_yrs)), get_obs_ests_multinomial(jags_data$carc_x_obs[observable,,j], 1), main = TeX("$Age\\,3\\,(NOR)$"), yrs = all_yrs[ts_yrs])
  plot_tseries(post_summ(post, sub_index("^q_Sa_adj[year,2,pop]$", pop = j, year = ts_yrs)), get_obs_ests_multinomial(jags_data$carc_x_obs[observable,,j], 2), main = TeX("$Age\\,4\\,(NOR)$"), yrs = all_yrs[ts_yrs])
  plot_tseries(post_summ(post, sub_index("^q_Sa_adj[year,3,pop]$", pop = j, year = ts_yrs)), get_obs_ests_multinomial(jags_data$carc_x_obs[observable,,j], 3), main = TeX("$Age\\,5\\,(NOR)$"), yrs = all_yrs[ts_yrs])
  plot_tseries(post_summ(post, sub_index("^q_Sa_adj[year,4,pop]$", pop = j, year = ts_yrs)), get_obs_ests_multinomial(jags_data$carc_x_obs[observable,,j], 4), main = TeX("$Age\\,3\\,(NOR)$"), yrs = all_yrs[ts_yrs])
  plot_tseries(post_summ(post, sub_index("^q_Sa_adj[year,5,pop]$", pop = j, year = ts_yrs)), get_obs_ests_multinomial(jags_data$carc_x_obs[observable,,j], 5), main = TeX("$Age\\,4\\,(NOR)$"), yrs = all_yrs[ts_yrs])
  plot_tseries(post_summ(post, sub_index("^q_Sa_adj[year,6,pop]$", pop = j, year = ts_yrs)), get_obs_ests_multinomial(jags_data$carc_x_obs[observable,,j], 6), main = TeX("$Age\\,5\\,(NOR)$"), yrs = all_yrs[ts_yrs])
  mtext(side = 1, outer = T, line = 0.25, "Return Year")
...
carc_comp_fit_plot_age = function(j) {
  par(mfcol = c(1,3), oma = c(1.5,1.5,0,0))
  plot_tseries(post_summ(post, sub_index("^q_Sa_adj_age[year,1,pop]$", pop = j, year = ts_yrs)), get_obs_ests_multinomial(x_carc_new_obs$x_carc_age_obs[observable,,j], 1), main = TeX("$Age\\,3$"), yrs = all_yrs[ts_yrs])
  plot_tseries(post_summ(post, sub_index("^q_Sa_adj_age[year,2,pop]$", pop = j, year = ts_yrs)), get_obs_ests_multinomial(x_carc_new_obs$x_carc_age_obs[observable,,j], 2), main = TeX("$Age\\,4$"), yrs = all_yrs[ts_yrs])
  plot_tseries(post_summ(post, sub_index("^q_Sa_adj_age[year,3,pop]$", pop = j, year = ts_yrs)), get_obs_ests_multinomial(x_carc_new_obs$x_carc_age_obs[observable,,j], 3), main = TeX("$Age\\,5$"), yrs = all_yrs[ts_yrs])
  mtext(side = 1, outer = T, line = 0.25, "Return Year")
  mtext(side = 2, outer = T, line = 0.25, "Proportion of Return")
}
...
carc_comp_fit_plot_origin = function(j) {
  par(mfcol = c(1,2), oma = c(1.5,1.5,0,0))
  plot_tseries(post_summ(post, sub_index("^q_Sa_adj_origin[year,1,pop]$", pop = j, year = ts_yrs)), get_obs_ests_multinomial(x_carc_new_obs$x_carc_origin_obs[observable,,j], 1), main = TeX("$NOR$"), yrs = all_yrs[ts_yrs])
  plot_tseries(post_summ(post, sub_index("^q_Sa_adj_origin[year,2,pop]$", pop = j, year = ts_yrs)), get_obs_ests_multinomial(x_carc_new_obs$x_carc_origin_obs[observable,,j], 2), main = TeX("$HOR$"), yrs = all_yrs[ts_yrs])
  mtext(side = 1, outer = T, line = 0.25, "Return Year")
  mtext(side = 2, outer = T, line = 0.25, "Proportion of Return")
}
...
plot_f = function(j) {
  ...
  f_tot = post_summ(post, sub_index("^f_tot[year,pop]", pop = j, year = ts_yrs))
...
plot_f = function(j) {
  ...
  # conversion rates
  Pb_per_f_tot = post_summ(post, sub_index("^Pb_per_f_tot[year,pop]", pop = j, year = ts_yrs))
...
beta_per = post_subset(post, "mu_beta_per_wul", TRUE)
...
est = post_summ(post, "mu_beta_per_wul")
colnames(est) = "slope"
...
plot_fn = function(j) {
  # extract parr, scale it to WUL, and create new post object
  Pa = post_subset(post, sub_index("^Pa[year,.,pop]", pop = j, year = ts_yrs), matrix = TRUE)
  Pa_scaled = Pa/jags_data$wul[j]
  ...
  phi_Pa_Mb_mean = array_format(post_summ(post, sub_index("^phi_Pa_Mb[year,.+,pop]", pop = j, year = ts_yrs))["mean",])
  phi_Pa_Mb_lwr = array_format(post_summ(post,  sub_index("^phi_Pa_Mb[year,.+,pop]", pop = j, year = ts_yrs))["2.5%",])
  phi_Pa_Mb_upr = array_format(post_summ(post,  sub_index("^phi_Pa_Mb[year,.+,pop]", pop = j, year = ts_yrs))["97.5%",])
...
mtext(side = 1, line = 1.5, outer = T, "LH-Specific Parr/WUL", cex = 1.2)
...
plot_fn = function(j) {
  plot_tseries(est = post_summ(post, sub_index("n_stray_tot[year,pop]", pop = j, year = ts_yrs)), yrs = all_yrs[ts_yrs], main = pops[j])
}
...
maturity_plot = function(j) {
  par(mfrow = c(2,2), oma = c(1.5,1.5,0,0))
  plot_tseries(post_summ(post, sub_index("^psi_O1_Rb[year,origin,pop]$", year = ts_yrs, origin = o_nor, pop = j)), main = "Age 3 (NOR)", yrs = all_yrs[ts_yrs], ylim = c(0,1))
  plot_tseries(post_summ(post, sub_index("^psi_O1_Rb[year,origin,pop]$", year = ts_yrs, origin = o_hor, pop = j)), main = "Age 3 (HOR)", yrs = all_yrs[ts_yrs], ylim = c(0,1))
  plot_tseries(post_summ(post, sub_index("^psi_O2_Rb[year,origin,pop]$", year = ts_yrs, origin = o_nor, pop = j)), main = "Age 4 (NOR)", yrs = all_yrs[ts_yrs], ylim = c(0,1))
  plot_tseries(post_summ(post, sub_index("^psi_O2_Rb[year,origin,pop]$", year = ts_yrs, origin = o_hor, pop = j)), main = "Age 4 (HOR)", yrs = all_yrs[ts_yrs], ylim = c(0,1))
...
my_boxplot("^sig_Lphi_f_Pb[.]$", "Process SD")
my_boxplot("^rho_Lphi_f_Pb", "Among-pop Correlation", ticklabs = vcov_labels, las = 3)
```
