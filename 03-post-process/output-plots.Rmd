...
weir_comp_fit_plot(1)
```

#### LOS

```{r lost-weir-comp-fit, fig.width = 8, fig.height = 5}
weir_comp_fit_plot(2)
```

#### MIN

```{r min-weir-comp-fit, fig.width = 8, fig.height = 5}
weir_comp_fit_plot(3)
```

#### UGR

```{r ugr-weir-comp-fit, fig.width = 8, fig.height = 5}
weir_comp_fit_plot(4)
```

### Carcass Sampling {.tabset .tabset-pills}

```{r carc-comp-fit-func}
...
carc_comp_fit_plot(1)
```

#### LOS

```{r lost-carc-comp-fit, fig.width = 8, fig.height = 5}
carc_comp_fit_plot(2)
```

#### MIN

```{r min-carc-comp-fit, fig.width = 8, fig.height = 5}
carc_comp_fit_plot(3)
```

#### UGR

```{r ugr-carc-comp-fit, fig.width = 8, fig.height = 5}
carc_comp_fit_plot(4)
```
...
This figure shows the inherent density dependence built in to the Beverton-Holt recruitment function: with increasing egg density, survival per egg decreases. The horizontal line/region shows the theoretical maximum ($\alpha_j$) in the absence of density dependence.
...
junk = sapply(1:jags_data$nj, plot_f)
mtext(side = 1, outer = T, line = 0.25, "Total Egg Production (Millions)")
mtext(side = 1, outer = T, line = 0.25, "Parr per Egg")
```
...
junk = sapply(1:jags_data$nj, plot_f)
mtext(side = 1, outer = T, line = 0.25, "Total Spawners")
mtext(side = 1, outer = T, line = 0.25, "Parr per Spawner")
```
