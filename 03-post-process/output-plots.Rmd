...
L2 = c("Intercept", "Slope", "Process SD", "Among-pop Correlation", "Realized")
base = list("gamma0[LH_type,pop]", "gamma1[LH_type,pop]", "sig_Lphi_Pa_Mb[pop,LH_type]", "rho_Lphi_Pa_Mb[.,.,LH_type]", "^phi_Pa_Mb[year,LH_type,pop]")
...
L2 = c("Mean", "Process SD", "Among-pop Correlation", "Realized")
base = c("mu_psi_O1[origin,pop]", "sig_Lpsi_O1[pop,origin]", "rho_Lpsi_O1[.,.,origin]", "^psi_O1[.+,origin,pop]")
...
L2 = c("Mean", "Process SD", "Among-pop Correlation", "Realized")
base = c("mu_psi_O2[origin,pop]", "sig_Lpsi_O2[pop,origin]", "rho_Lpsi_O2[.,.,origin]", "^psi_O2[.+,origin,pop]")
match = sub_index(base, year = ".+", origin = o_nor, pop = ".")
tmp_diags = lapply(1:length(match), function(i) summarize_posterior(match[i], L1 = L1, L2 = L2[i], L3 = "NOR"))
tmp_summ = lapply(tmp_diags, function(i) summarize_diags(i))
diags_out = rbind(diags_out, do.call(rbind, tmp_diags))
...
### "Growth Factor" vs. Summer Length
...
E.g., for some arbitrary survival term `phi` which has logit-normal process noise around the expected value `mu_phi` with logit-scale standard deviation of `sigma_phi`:
...
for (y in 1:ny) {
  # random process
  Lphi[y] ~ dnorm(logit(mu_phi), 1/sigma_phi^2)
  phi[y] <- ilogit(Lphi[y])

  # standardized noise term
  phi_qresid[y] <- pnorm(logit(phi[y]), logit(mu_phi), 1/sigma_phi^2)
}
```

Continuing with this example, suppose we have counts of tagged fish we know survived (`x_surv[y]`) and the total number of fish tagged (`n_tagged[y]`). We can calculate the noise term of `x_surv[y]` when assuming it is a binomial random variable using:
...
plot_proc_qresid = function(qresid_param, year = ts_yrs, pop = NULL, origin = NULL, LH_type = NULL, age = NULL, main = NULL) {
  ...
  qresid = apply(qresid_post, 2, function(x) c(mean = mean(x), sd = sd(x), quantile(x, c(0.5, 0.025, 0.975))))
  ...
  par(lend = "square")
  plot(qresid["mean",] ~ all_yrs[y_ind], main = main, type = "n",
  ...
  segments(all_yrs[observable], qresid["2.5%",], all_yrs[observable], qresid["97.5%",], col = "red")
  points(qresid["mean",] ~ all_yrs[observable], pch = 21, col = "red", bg = alpha("salmon", 0.5), cex = 1.2)
  
  # draw legend that shows estimate summaries
  usr = par("usr"); xdiff = diff(usr[1:2]); ydiff = diff(usr[3:4])
  rect(usr[1], 1, usr[2], usr[4], border = "black", col = "grey90")
  legend(x = usr[1] - xdiff * 0.05, y = usr[4] + ydiff * 0.025, legend = make_legend_text(mn_post, sd_post, first_last_mn_diff_post), bty = "n", cex = 0.8)
  box()
...
my_par()
tmp = lapply(1:jags_data$nj, function(j) plot_proc_qresid("Lpi_qresid[year,pop]", pop = j, main = pops[j]))
...
#### Summer Parr to Spring Smolt "Growth Factor"
...
oma_labels("Brood Year", "log(Summer to Spring Growth Factor) Residual")
proc_error_ests = rbind(proc_error_ests, do.call(rbind, tmp))
```
...
my_par()
tmp = lapply(1:jags_data$nj, function(j) plot_proc_qresid("Lphi_Mb_Ma_qresid[year,origin,pop]", pop = j, origin = 1, main = pops[j]))
...
my_par()
tmp = lapply(1:jags_data$nj, function(j) plot_proc_qresid("Lphi_Mb_Ma_qresid[year,origin,pop]", pop = j, origin = 2, main = pops[j]))
...
```{r LGR-to-BON-surv-noise, fig.width = 6, fig.height = 3.5}
...
```{r BON-to-LGR-surv-noise, fig.width = 6, fig.height = 3.5}
my_par(); par(mfrow = c(1,2))
...
my_par()
tmp = lapply(1:jags_data$nj, function(j) plot_proc_qresid("Lphi_Sb_Sa_qresid[year,pop]", pop = j, main = pops[j]))
...
proc_error_ests$param = stringr::str_replace(proc_error_ests$param, "Lpi_qresid\\[year,pop\\]", "Migratory Strategy Apportionment")
...
proc_error_ests$param = stringr::str_replace(proc_error_ests$param, "Lphi_Mb_Ma_qresid\\[year,origin,pop\\]", "Trib to LGR Survival")
...
proc_error_ests$param = stringr::str_replace(proc_error_ests$param, "Lphi_Sb_Sa_qresid\\[year,pop\\]", "Pre-spawn Survival")
...
plot_obs_qresid = function(qresid_param, year = ".+", pop = NULL, origin = NULL, LH_type = NULL, age = NULL, main = NULL) {
  ...
  empty = matrix(NA, nrow = 5, ncol = length(obs_qresid_yrs))
  empty[,y_ind] = qresid
  qresid = empty[,observable]
  rownames(qresid) = c("mean", "sd", "50%", "2.5%", "97.5%")
  ...
  mn_post_first = apply(qresid_post[,i_first], 1, mean)
  mn_post_last = apply(qresid_post[,i_last], 1, mean)
  first_last_mn_diff_post = mn_post_last - mn_post_first
  ...
  # blank plot with correct dimensions
  par(lend = "square")
  plot(qresid["mean",] ~ all_yrs[observable], main = main,  type = "n", ylim = ylim, xlim = range(all_yrs[observable]), xaxt = "n", xlab = "", ylab = "",yaxt = "n")
...
```{r hydro-surv-resid, fig.width = 6, fig.height = 3.5}
my_par(); par(mfrow = c(1,2))
