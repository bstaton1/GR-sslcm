...
WUL_change_fig_caption = "
  Density-dependent survival relationships from egg (panels a-d) and parr (panels e-f) to smolt arriving at Lower Granite Dam (LGR) under current (solid lines) and increased (dashed lines) weighted usable habitat (WUL).
  Egg $\\to$ LGR survival includes density effects that are both direct (e.g., egg survival varies as a function of egg density) and indirect (parr size varies as a function of egg density, and survival varies with size), whereas parr $\\to$ LGR survival includes only the indirect density effects.
"
```
...
```{r WUL-change-fig, fig.width = 7.2, fig.height = 4.5, eval = params$WUL_change_fig, fig.cap = WUL_change_fig_caption, time_it = TRUE}

prep_samples = function(j) {
  
  # the parameters needed to calucate process model expected values
...
get_phi_hat = function(x, E, WUL_change = 0, start = "E") {
...
get_phi_hat_post = function(j, E_pred, WUL_change = 0, start = "E") {
  
  the_samps = prep_samples(j = j)
  
  out = sapply(1:nrow(the_samps), function(i) {
    x = prep_sample(the_samps, i = i, j = j)
    sapply(E_pred, function(e) get_phi_hat(x, E = e, WUL_change = WUL_change, start = start))
  })
...
plot_f = function(j, start, WUL_change) {
  ...
  phi_hat_post = get_phi_hat_post(j = j, E_pred, start = start)
  phi_hat = post_summ(phi_hat_post, ".")
  phi_hat_new_post = get_phi_hat_post(j = j, E_pred, WUL_change = WUL_change, start = start)
  phi_hat_new = post_summ(phi_hat_new_post, ".")
  
  plot(phi_real["mean",] ~ E_real["mean",], type = "n",
       ylim = make_lim(0, phi_hat[c("2.5%","97.5%"),],
                phi_hat_new[c("2.5%","97.5%"),]),
                # phi_real["mean",]),
       # ylim = make_lim(0, ifelse(start == "E", 0.075, 0.3)),
       xlim = c(0, max(E_real["mean",], E_pred)),
       xaxt = "n")
  
  # points(phi_real["mean",] ~ E_real["mean",], pch = 21, 
  #        col = "black", bg = scales::alpha("grey25", 0.25), cex = 0.8)
  
  polygon(x = c(E_pred, rev(E_pred)), 
          y = c(phi_hat["2.5%",], rev(phi_hat["97.5%",])),
          col = tranp_col, border = NA)
  lines(phi_hat["mean",] ~ E_pred, lwd = 2)
  lines(phi_hat["2.5%",] ~ E_pred)
  lines(phi_hat["97.5%",] ~ E_pred)
  
  polygon(x = c(E_pred, rev(E_pred)), 
          y = c(phi_hat_new["2.5%",], rev(phi_hat_new["97.5%",])),
          col = tranp_col, border = NA)
  lines(phi_hat_new["mean",] ~ E_pred, lwd = 2, lty = 2)
  lines(phi_hat_new["2.5%",] ~ E_pred,  lty = 2)
  lines(phi_hat_new["97.5%",] ~ E_pred, lty = 2)
  at_x = axisTicks(par("usr")[1:2], log = FALSE)
  axis(side = 1, at = at_x, labels = at_x/1e6)
  panel_label(paste0("(", letters[ifelse(start == "E", 0, 3) + j], ") ", pops[j]))
  
  if (j == 1) {
    if (start == "E") {
      axis_labels(ylab = "Egg \u2192 LGR Survival", outer = FALSE, yline = 1)
      legend("topright", title = "WUL", legend = c("Current", paste0("\u2191", WUL_change * 100, "%")), lty = c(1,2), seg.len = 1.5, bty = "n", cex = 0.8)
    } 
    if (start == "Pb") axis_labels(ylab = "Parr \u2192 LGR Survival", outer = FALSE, yline = 1)
  }
...
mypar(mfrow = c(2,4), col.axis = "black")
junk = sapply(1:4, function(j) plot_f(j, "E", 1))
junk = sapply(1:4, function(j) plot_f(j, "Pb", 1))
axis_labels(xlab = "Egg Production (Millions)")
```
