...
# Assumed-Known Quantities {.tabset .tabset-dropdown}
...
plot_f = function(ko) {
  ...
  ylim = c(0, max(jags_data$U, na.rm = TRUE))
...
Where `yes` and `no` are vectors storing as elements the number of female carcasses examined in a given return year (`year`) for one population (`one_pop`) that were and were not successful in spawning ("successful" spawning is assigned by the absence of >50% the potential total eggs carried by that female).
Values were bounded on the interval [0.4,1); several years for UGR suggested very low survival with low sample size.

</details>

```{r known-phi-Sb-Sa-fn}
