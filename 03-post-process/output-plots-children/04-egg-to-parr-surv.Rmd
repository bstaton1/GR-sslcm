...
```{r phi_E_Pb-plots, fig.width = 6, fig.height = 5}
plot_f = function(j) {
  plot_tseries(post_summ(post, sub_index("^phi_E_Pb[year,pop]$", year = ts_yrs, pop = j)),
               yrs = all_yrs[ts_yrs],
               main = pops[j])
}
...
mtext(side = 2, outer = T, line = 0.25, "Egg-to-Parr Survival")
```
...
```{r phi_E_Pb_dot-plots, fig.width = 6, fig.height = 5}
plot_f = function(j) {
  plot_tseries(post_summ(post, sub_index("^phi_E_Pb[year,pop]$", year = ts_yrs, pop = j)),
               yrs = all_yrs[ts_yrs],
               main = pops[j])
  
  # draw on the posterior time series that includes BH prediction only
  dot_col = "royalblue"
  dot = post_summ(post, sub_index("^phi_E_Pb_dot[year,pop]", pop = j, year = ts_yrs))
  polygon(x = c(all_yrs[ts_yrs], rev(all_yrs[ts_yrs])),
          y = c(dot["2.5%",], rev(dot["97.5%",])),
          col = scales::alpha(dot_col, 0.15), border = NA)
  lines(dot["mean",] ~ all_yrs[ts_yrs], col = dot_col, lwd = 2)
  lines(dot["2.5%",] ~ all_yrs[ts_yrs], col = dot_col, lty = 2)
  lines(dot["97.5%",] ~ all_yrs[ts_yrs], col = dot_col, lty = 2)
}
...
```{r phi_E_Pb_dot2-plots, fig.width = 6, fig.height = 5}
plot_f = function(j) {
  plot_tseries(post_summ(post, sub_index("^phi_E_Pb[year,pop]$", year = ts_yrs, pop = j)),
               yrs = all_yrs[ts_yrs],
               main = pops[j])
  
  # draw on the posterior time series that includes BH prediction and the carry over residual from last year
  dot_col = "royalblue"
  dot = post_summ(post, sub_index("^phi_E_Pb_dot2[year,pop]", pop = j, year = ts_yrs))
  polygon(x = c(all_yrs[ts_yrs], rev(all_yrs[ts_yrs])),
          y = c(dot["2.5%",], rev(dot["97.5%",])),
          col = scales::alpha(dot_col, 0.15), border = NA)
  lines(dot["mean",] ~ all_yrs[ts_yrs], col = dot_col, lwd = 2)
...
```{r egg-surv-vs-eggs, fig.width = 6, fig.height = 5}
plot_f = function(j) {
  ...
  # empty plot
  plot(1, 1, type = "n", xlim = range(0, E[5,]), ylim = range(0, phi_E_Pb[5,], phi_E_Pb_pred[5,]),
  ...
  polygon(c(E_pred, rev(E_pred)), c(phi_E_Pb_pred[4,], rev(phi_E_Pb_pred[5,])), col = alpha("salmon", 0.5), border = NA)
  lines(phi_E_Pb_pred[4,] ~ E_pred, col = "red", lty = 2)
  lines(phi_E_Pb_pred[5,] ~ E_pred, col = "red", lty = 2)
  lines(phi_E_Pb_pred[3,] ~ E_pred, col = "red", lwd = 2)
  
  # draw state pairs + uncertainty
  segments(E[4,], phi_E_Pb[3,], E[5,], phi_E_Pb[3,], col = alpha("red", 0.5))
  segments(E[3,], phi_E_Pb[4,], E[3,], phi_E_Pb[5,], col = alpha("red", 0.5))
  points(phi_E_Pb[3,] ~ E[3,], pch = 16, col = alpha("salmon", 0.5), cex = 1.8)
  text(phi_E_Pb[3,] ~ E[3,], labels = substr(all_yrs[ts_yrs], 3, 4), cex = 0.65, col = "black")
}
...
plot_f = function(j) {
  ...
  points(Pb[3,] ~ E[3,], pch = 16, col = alpha("salmon", 0.5), cex = 1.8)
  text(Pb[3,] ~ E[3,], labels = substr(all_yrs[ts_yrs], 3, 4), cex = 0.65, col = "black")
}
...
plot_f = function(j) {
  ...
  points(E["mean",] ~ Sa_tot["mean",], pch = 16, col = alpha("salmon", 0.5), cex = 1.8)
