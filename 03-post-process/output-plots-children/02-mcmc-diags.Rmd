...
All convergence diagnostics were calculated using the '[posterior](https://mc-stan.org/posterior/articles/posterior.html)' R package, which implements updated versions (based on rank-normalization) of the classical convergence diagnostics (presented in Vehtari et al. [2021](https://projecteuclid.org/journals/bayesian-analysis/advance-publication/Rank-Normalization-Folding-and-Localization--An-Improved-R%cb%86-for/10.1214/20-BA1221.full)).

* **Rhat** measures convergence of multiple chains to a common target distribution. Values close to 1 are better -- we've chosen these approximate categories for ease of interpretation: 
  * _Ideal_: [1,1.01] 
...
rhat_hist = function(rhat) {
  ...
  axis_labels(xlab = TeX("$\\hat{R}$"), ylab = "Frequency")
  usr = par("usr")
