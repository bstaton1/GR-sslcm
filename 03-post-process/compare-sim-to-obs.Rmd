...
for (i in 1:nrow(Ra)) {
  ...
  out_3 = rbind(out_3, as.numeric(p_hor_3))
  out_4 = rbind(out_4, as.numeric(p_hor_4))
  out_5 = rbind(out_5, as.numeric(p_hor_5))
}

# build the row indices for each element: the year
r_ind = rep(1:nrow(p_hor_3), ncol(p_hor_3))

# build the column indices for each element: the population
c_ind = rep(1:ncol(p_hor_3), each = nrow(p_hor_3))

# make element names
colnames(out_3) = paste0("p_hor_a[", r_ind, ",1,", c_ind, "]")
colnames(out_4) = paste0("p_hor_a[", r_ind, ",2,", c_ind, "]")
colnames(out_5) = paste0("p_hor_a[", r_ind, ",3,", c_ind, "]")
out = cbind(out_3, out_4, out_5)
...
# for each posterior sample, calculate the abundance by origin and total
out_tot = out_nor = out_hor = NULL
for (i in 1:nrow(Rb)) {
  ...
  out_tot = rbind(out_tot, as.numeric(tot))
}
...
# make element names
colnames(out_nor) = paste0("Rb_tot_org[", r_ind, ",1,", c_ind, "]")
colnames(out_hor) = paste0("Rb_tot_org[", r_ind, ",2,", c_ind, "]")
colnames(out_tot) = paste0("Rb_tot[", r_ind, ",", c_ind, "]")
out = cbind(out_nor, out_hor, out_tot)
...
# make element names
colnames(out_nor) = paste0("Ra_tot_org[", r_ind, ",1,", c_ind, "]")
colnames(out_hor) = paste0("Ra_tot_org[", r_ind, ",2,", c_ind, "]")
...
for (i in 1:nrow(Ra)) {
  ...
  nor = Sa_i[,1,1,] + Sa_i[,2,1,] + Sa_i[,3,1,]
  hor = Sa_i[,1,2,] + Sa_i[,2,2,] + Sa_i[,3,2,]
...
# make element names
colnames(out_nor) = paste0("Sa_tot_org[", r_ind, ",1,", c_ind, "]")
colnames(out_hor) = paste0("Sa_tot_org[", r_ind, ",2,", c_ind, "]")
out = cbind(out_nor, out_hor)
...
for (i in 1:nrow(Ra)) {
  ...
  E_nor_i = E_i[,1,1,] + E_i[,2,1,] + E_i[,3,1,]
  E_hor_i = E_i[,1,2,] + E_i[,2,2,] + E_i[,3,2,]

  out_nor = rbind(out_nor, as.numeric(E_nor_i))
  out_hor = rbind(out_hor, as.numeric(E_hor_i))
}
...
# make element names
colnames(out_nor) = paste0("E_org[", r_ind, ",1,", c_ind, "]")
colnames(out_hor) = paste0("E_org[", r_ind, ",2,", c_ind, "]")
out = cbind(out_nor, out_hor)
...
for (i in 1:nrow(Ra)) {
  Sa_i = array_format(Sa[i,])
  Sa_nor_i = Sa_i[,1,1,] + Sa_i[,2,1,] + Sa_i[,3,1,]
  Sa_hor_i = Sa_i[,1,2,] + Sa_i[,2,2,] + Sa_i[,3,2,]
  E_i = array_format(E[i,])
  E_nor_i = E_i[,1,1,] + E_i[,2,1,] + E_i[,3,1,]
  E_hor_i = E_i[,1,2,] + E_i[,2,2,] + E_i[,3,2,]

  out_nor = rbind(out_nor, as.numeric(E_nor_i/Sa_nor_i))
  out_hor = rbind(out_hor, as.numeric(E_hor_i/Sa_hor_i))
}
...
# make element names
colnames(out_nor) = paste0("E_per_Sa_org[", r_ind, ",1,", c_ind, "]")
colnames(out_hor) = paste0("E_per_Sa_org[", r_ind, ",2,", c_ind, "]")
out = cbind(out_nor, out_hor)
...
p3_out = p4_out = p5_out = NULL
for (i in 1:nrow(Ra)) {
  Ra_i = array_format(Ra[i,])
  
  a3_tot = Ra_i[,1,1,] + Ra_i[,1,2,]
  a4_tot = Ra_i[,2,1,] + Ra_i[,2,2,]
  a5_tot = Ra_i[,3,1,] + Ra_i[,3,2,]
  tot = a3_tot + a4_tot + a5_tot
  
  p3 = Ra_i[,1,1,]/tot
  p4 = Ra_i[,2,1,]/tot
  p5 = Ra_i[,3,1,]/nor_tot

  p3_out = rbind(p3_out, as.numeric(p3))
  p4_out = rbind(p4_out, as.numeric(p4))
  p5_out = rbind(p5_out, as.numeric(p5))
}

# build the row indices for each element: the year
r_ind = rep(1:nrow(p3), ncol(p3))

# build the column indices for each element: the population
c_ind = rep(1:ncol(p3), each = nrow(p3))

# make element names
colnames(p3_out) = paste0("Ra_comp_a[", r_ind, ",", "1", ",", c_ind, "]")
colnames(p4_out) = paste0("Ra_comp_a[", r_ind, ",", "2", ",", c_ind, "]")
colnames(p5_out) = paste0("Ra_comp_a[", r_ind, ",", "3", ",", c_ind, "]")
...
assign_color_1 = function(x) {
  ...
  breaks = c(0, 0.25, 0.5, 0.75, 0.9, 1.1, 1.25, 1.5, 1.75, 100)
...
prep_est_ratios = function(x, q) {
  ...
  x$Type = factor(x$Type, levels = c("Egg to Parr Survival", "Parr Mean Size", "Proportion Fall Migrants", "Overwinter Survival", "Parr to Smolt Mean Size 'Growth Factor'", "Smolt Mean Size", "Migration to LGR Survival", "LGR to Ocean Survival", "First Year Ocean Survival", "Age-3 Maturity", "Age-4 Maturity", "BON to LGR Survival", "Pre-Spawn Survival"))
...
For us to use the state-space model as a simulation model (which is the ultimate intent), it would be desirable if several characteristics of the distribution of stochastic and derived quantities were similar between the time periods with and without observations.
